...
These are canonical role names. The actual label strings used in the issue tracker may differ. The mapping should have been provided to you. If not, follow the installed `mattpocock-skills-codex:setup-matt-pocock-skills` skill if the current task authorizes setup, or ask the user to invoke it.
...
4. **Grill (if needed).** If the request needs fleshing out, follow the installed `mattpocock-skills-codex:grilling` and `mattpocock-skills-codex:domain-modeling` skills in sequence, and grill it into shape a round of questions at a time, sharpening domain terms and updating `CONTEXT.md`/ADRs inline as decisions land.
