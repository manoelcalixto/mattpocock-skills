...
Wayfinder uses the shared Planning context when an effort crosses sessions. Before creating the map, follow the installed `mattpocock-skills-codex:planning-context` skill to initialize the effort ledger and create an intermediate checkpoint. Keep the map and its tickets as tracker pointers to that ledger.

When a Decision ticket resolves a material choice, inspect the active entries first. Run `decision reference --effort <effort> --decision DEC-NNN` when one active entry already represents the answer, or run `decision add` once when it does not. Use the returned stable ID exactly once for that ticket. If the answer is ADR-worthy, follow the installed `mattpocock-skills-codex:domain-modeling` skill, make the ADR canonical, and pass its path with `decision add --adr`; the ledger and ticket point to the ADR without repeating its rationale.
...
**Where the map, its child tickets, blocking, and frontier queries physically live is tracker-specific.** The issue tracker should have been provided to you. If not, follow the installed `mattpocock-skills-codex:setup-matt-pocock-skills` skill if the current task authorizes setup, or ask the user to invoke it. Consult the tracker doc's "Wayfinding operations" section for how _this_ repo expresses them. If no tracker has been provided, default to the local-markdown tracker.
...
- **Research** (AFK): Reading documentation, third-party APIs, or local resources like knowledge bases to surface a fact a decision waits on. When delegation is available and permitted, use a native Codex subagent that follows the installed `mattpocock-skills-codex:research` skill. Use when knowledge outside the current working directory is required.
- **Prototype** (HITL): Raise the fidelity of the discussion by making a cheap, rough, concrete artifact to react to (an outline, a rough take, a stub, or UI/logic code) by following the installed `mattpocock-skills-codex:prototype` skill. Links the prototype as an asset. Use when "how should it look" or "how should it behave" is the key question.
- **Grilling** (HITL): Conversation. The default case. Follow the installed `mattpocock-skills-codex:grilling` and `mattpocock-skills-codex:domain-modeling` skills as separate steps.
...
1. **Name the destination.** Follow the installed `mattpocock-skills-codex:grilling` and `mattpocock-skills-codex:domain-modeling` skills as separate steps to pin down what this map is finding its way to: the spec, decision, or change. The destination fixes the scope, so it's settled first.
2. **Map the frontier.** Grill again, **breadth-first** this time: fan out across the whole space rather than deep on any one thread, surfacing the open decisions and the first steps takeable now. **If this surfaces no fog** (the way to the destination is already clear, the whole journey small enough for one session), you don't need a map. Stop and ask the user how they'd like to proceed.
3. **Initialize the Planning context.** Follow the installed `mattpocock-skills-codex:planning-context` skill, create the per-effort ledger, and create an intermediate checkpoint before the map is published. Add the returned marker to the map's Planning context section.
...
6. **Resolve research tickets.** For each `research` ticket you just created, use native Codex subagents when delegation is available and permitted, following `mattpocock-skills-codex:research` and the current concurrency limits. Capture findings on a throwaway `research/<name>` branch with a context pointer from the ticket. If subagents are unavailable, resolve the research directly.
...
3. Resolve it. **Zoom as needed**: fetch the full body of any related or closed ticket on demand; follow the installed plugin skills named in the `## Notes` block. If in doubt, follow `mattpocock-skills-codex:grilling` and `mattpocock-skills-codex:domain-modeling` as separate steps.
