...
Include a "suggested skills" section in the document, naming which installed skills the next agent should read and follow.

When the destination is a fresh session and the current work has an active Planning context, follow the installed `mattpocock-skills-codex:planning-context` skill first and create the checkpoint that matches the next phase. Use an intermediate checkpoint while planning continues, a final checkpoint before implementation, and an implementation checkpoint after verification.
