...
If an active decision cannot be honored, stop the implementation at that conflict and report the decision ID and consequence. Follow the installed `mattpocock-skills-codex:planning-context` skill to return to planning for user resolution. Resume only after a superseding decision, a new Planning checkpoint, a refreshed marker, and a passing preflight. Keep the conflict explicit instead of recording a silent deviation in code, tests, commits, or the ledger.

Record the current `HEAD` SHA before implementation. It is the fixed point for this invocation.

Follow the installed `mattpocock-skills-codex:tdd` skill where possible, at pre-agreed seams.
...
Follow the installed `mattpocock-skills-codex:code-review` skill once, using the starting SHA as its fixed point. Check each finding's citation, then apply the applicable findings in one **fix batch**. Documented-standard violations and spec gaps can require a fix; baseline smells are judgement calls. For a marked path, put one observable `Planning-Verification: DEC-NNN | <evidence>` trailer on an implementation or fix commit for each decision whose behavior that commit verifies or changes. Re-run the relevant validation and commit the fix batch if it changed the code.
