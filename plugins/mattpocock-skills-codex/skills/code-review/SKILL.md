---
name: code-review
description: "Review a stable code checkpoint against repository standards and the originating specification. Use for a branch, PR, or fixed-point diff."
---
...
Keep the axes independent. When native Codex subagents are available and the current instructions authorize delegation, assign each bounded axis to one subagent, respecting the current concurrency limit. Otherwise review each axis directly in a separate pass and label the results as a self-review. One initial pass completes the checkpoint. Commits that apply its findings remain part of that checkpoint and do not start another review automatically.

The issue tracker should have been provided to you. If `docs/agents/issue-tracker.md` is missing, follow the installed `mattpocock-skills-codex:setup-matt-pocock-skills` skill if the current task authorizes setup, or ask the user to invoke it.

## Process

### 1. Pin the fixed point and checkpoint

Use the fixed point the user supplied (a commit SHA, branch name, tag, `main`, `HEAD~5`, etc.). If none was supplied, resolve the branch's merge-base with its configured base branch when unambiguous; ask only when no reliable base can be determined.
...
### 4. Run the eligible axes independently
...
If the spec is missing, skip the Spec axis and note this in the final report. Collect both independent reports before changing the code or combining conclusions.
