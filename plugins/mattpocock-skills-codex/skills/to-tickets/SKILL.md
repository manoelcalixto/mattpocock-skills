...
The issue tracker and triage label vocabulary should have been provided to you. If not, follow the installed `mattpocock-skills-codex:setup-matt-pocock-skills` skill if the current task authorizes setup, or ask the user to invoke it.

When the source conversation or specification has an active Planning context, follow the installed `mattpocock-skills-codex:planning-context` skill before drafting. It is the sole owner of the ledger, checkpoint, coverage, and marker contract. A source without a Planning context marker follows the legacy ticket path.
...
Publish the approved tickets. **How** depends on the tracker configured by `mattpocock-skills-codex:setup-matt-pocock-skills`; the tickets are the same either way, only the shape of the blocking edges changes:
