...
When the source conversation has an active Planning context, follow the installed `mattpocock-skills-codex:planning-context` skill before drafting. It is the owner of the ledger, checkpoint, coverage, and marker contract. A source without a Planning context marker follows the legacy path and does not need a ledger.
