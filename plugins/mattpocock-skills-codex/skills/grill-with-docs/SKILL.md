...
Read and follow the installed `mattpocock-skills-codex:planning-context` skill to initialize the repository's Planning discovery and resolve the effort ledger. Then read and follow `mattpocock-skills-codex:grilling`, followed by `mattpocock-skills-codex:domain-modeling`. Keep these as separate steps because each skill owns a different part of the flow. Resolve each companion through the installed skills catalog; Codex does not require a separate Skill tool.
