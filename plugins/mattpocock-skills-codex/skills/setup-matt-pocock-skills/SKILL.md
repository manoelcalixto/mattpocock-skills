...
After the repository choices are confirmed, follow the installed `mattpocock-skills-codex:planning-context` skill to initialize the default Planning context discovery. It creates `docs/agents/planning.md` for a new repository and lazily appends the versioned block to an existing file without replacing its content.
