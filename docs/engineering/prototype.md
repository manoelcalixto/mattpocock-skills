...
```bash
npx skills add manoelcalixto/mattpocock-skills --skill=prototype
```

Select Codex when prompted. Use `npx skills update prototype` to refresh an editable install.

[Skill source](../../skills/engineering/prototype/SKILL.md)
...
`prototype` is a reach-for-it-anytime standalone: you drop into it to resolve a design question, then drop back out. Its answer often feeds the next step — a validated state model or UI direction becomes settled input for [to-spec](../engineering/to-spec.md) to write up, or an architectural decision worth recording via [domain-modeling](../engineering/domain-modeling.md). When you're unsure which skill or flow fits, [ask-matt](../engineering/ask-matt.md) routes you.
