...
Start a new Codex thread and type `$mattpocock-skills:prototype`.

[Source](https://github.com/manoelcalixto/mattpocock-skills/tree/main/skills/engineering/prototype)
...
Type `$mattpocock-skills:prototype`, or the agent reaches for it automatically when a task fits.
...
Picking the wrong branch wastes the whole prototype, so the question comes first. When the surrounding code cannot settle it safely, the two-way branch choice appears as a native Codex prompt when available. Both branches keep state in memory, run from one command, and surface the full state on every step.
