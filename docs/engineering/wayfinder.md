...
Every ticket is **HITL** (human in the loop — grilling, prototype) or **AFK** (agent alone — research); a HITL ticket only resolves through a live exchange, so the agent never answers its own questions. Research stays a real shared blocker, but charting dispatches takeable research tickets through [orchestrate-agents](https://aihero.dev/skills-orchestrate-agents) as bounded, non-recursive GPT-5.6 Sol batches starting at medium effort. Workers write unique reports in the shared working directory; the root alone resolves tickets and updates the map once. Parallel research is part of charting's sidecar semantics, so if MultiAgent V2 is unavailable, research tickets remain open for separate `$mattpocock-skills:research` threads instead of blocking charting inline.
...
- A human-worked `/new` session resolves **at most one ticket** (research sidecars excepted), records the answer as a resolution comment, closes the ticket, and appends a one-line pointer to *Decisions so far*.
