...
```bash
npx skills add manoelcalixto/mattpocock-skills --skill=ask-matt
```

Select Codex when prompted. Use `npx skills update ask-matt` to refresh an editable install.

[Skill source](../../skills/engineering/ask-matt/SKILL.md)
...
`ask-matt` is the **router** — the standalone map that sits over the whole set. It is the node every other docs page links back to as [ask-matt](../engineering/ask-matt.md), so it never sits *in* a chain; it points *into* every chain. From here you'll most often land on [grill-with-docs](../engineering/grill-with-docs.md), the head of the main flow, or [triage](../engineering/triage.md), the on-ramp for work you didn't create. When even the router's own picture is stale, its [Source](../../skills/engineering/ask-matt/SKILL.md) is the map of record.
