...
```bash
npx skills add manoelcalixto/mattpocock-skills --skill=implement
```

Select Codex when prompted. Use `npx skills update implement` to refresh an editable install.

[Skill source](../../skills/engineering/implement/SKILL.md)
...
The idea `implement` runs on is the **seam** — the stable interface a feature is tested at, chosen before any code is written. It doesn't invent seams mid-build; it uses the ones already picked (during [to-spec](../engineering/to-spec.md)) and writes tests against them via [tdd](../engineering/tdd.md). Working at pre-agreed seams is what keeps the implementation honest: the tests target something durable, so the code underneath can move without the tests moving.
...
Reach for it after the work has been specced and sequenced, not before. Its key neighbours are [to-tickets](../engineering/to-tickets.md), which produces the tickets — each declaring its blocking edges — that it works through, and [tdd](../engineering/tdd.md), which it drives internally to write the tests at each seam before running its own [code-review](../engineering/code-review.md) pass and committing. When you're unsure which skill or flow fits, [ask-matt](../engineering/ask-matt.md) routes you.
