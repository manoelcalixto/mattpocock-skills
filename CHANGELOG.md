...
- [#551](https://github.com/mattpocock/skills/pull/551) [`697d4ce`](https://github.com/manoelcalixto/mattpocock-skills/commit/697d4ce9742da558fd1ba6697c8e9775e2e302dd) Thanks [@mattpocock](https://github.com/mattpocock)! - Make the fork Codex-native and installable from its own marketplace as `mattpocock-skills@manoelcalixto`.
...
- [#2](https://github.com/manoelcalixto/mattpocock-skills/pull/2) [`6339cbe`](https://github.com/manoelcalixto/mattpocock-skills/commit/6339cbe226fd89ba85fcd2f217b7ad6e2e44706c) Thanks [@manoelcalixto](https://github.com/manoelcalixto)! - Make every skill available for automatic model invocation.

  All `agents/openai.yaml` files now omit the opt-out policy, promoted-skill docs and README sections describe both automatic and explicit invocation, and repository validation prevents the policy from being reintroduced.

- Standardize delegated workflows on Codex MultiAgent V2 and GPT-5.6 Sol, with task-shaped reasoning effort, explicitly authorized recursive delegation, and event-driven mailbox waiting.
...
- [#535](https://github.com/mattpocock/skills/pull/535) [`e74fee8`](https://github.com/manoelcalixto/mattpocock-skills/commit/e74fee89feb6025a6a74f6282feb7d33b1b6e578) Thanks [@mattpocock](https://github.com/mattpocock)! - Make `$mattpocock-skills:ask-matt` clued-up about `$mattpocock-skills:wayfinder` — the heaviest, most cognitively demanding flow.

  The router now sharpens the two routing mistakes people most often make with wayfinder:

  - **Over-reaching for it.** It's slower and denser than a single grill, so it's flagged as the heaviest flow and reserved for the idea that genuinely won't fit one session — a well-scoped feature belongs on `$mattpocock-skills:grill-with-docs`, not here.
  - **Losing the way at the handoff.** When the map clears, wayfinder hands off, it doesn't build: merge onto the main flow at `$mattpocock-skills:to-spec` (which collapses the map's linked decisions into a buildable plan) rather than looping the map straight into `$mattpocock-skills:implement`. Straight-to-`$mattpocock-skills:implement` is only for efforts that turned out genuinely small.

- [#502](https://github.com/mattpocock/skills/pull/502) [`44eed54`](https://github.com/manoelcalixto/mattpocock-skills/commit/44eed545186ffd0263e8004867750b80cfddd215) Thanks [@mattpocock](https://github.com/mattpocock)! - Make `$mattpocock-skills:setup-matt-pocock-skills` friendlier and align the local-markdown tracker with the current spec.
  ...
  - **Local-markdown tickets** are now one file per ticket under `.scratch/<feature>/issues/<NN>-<slug>.md` — never a single combined `tickets.md`. `$mattpocock-skills:to-tickets` and the local issue-tracker template now agree, and the spec file is `spec.md` (not `PRD.md`) to match `$mattpocock-skills:to-spec`.
...
- [#534](https://github.com/mattpocock/skills/pull/534) [`7d694b7`](https://github.com/manoelcalixto/mattpocock-skills/commit/7d694b7ae981ca221a8f759b15273fe7b5dc393e) Thanks [@mattpocock](https://github.com/mattpocock)! - Name the `$mattpocock-skills:wayfinder` unit a **decision ticket**.
