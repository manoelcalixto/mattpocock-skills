...
The owner scans the supplied history for repeatable `Planning-Verification` trailers, accepts already-read ticket evidence with an explicit origin, rejects unmerged or wrong-lineage tips and worker ledger edits, and checks every selected active decision that declares `verification` before writing. It must fail without modifying the ledger when any selected verification decision is absent. Every implementation and fix commit on the marked path must carry trailers only for decisions whose behavior it verifies or changes, and the union of the final history and validated ticket evidence must cover every selected decision that declares `verification` before aggregation. Decisions without that obligation are filtered from the verification set, while any evidence record naming one is rejected. Ticket-only mode remains valid, and the runtime gate fails closed whenever a selected verification decision lacks evidence. Only after aggregation succeeds, run the owner `checkpoint --phase implementation`. When this invocation owns only the applicable preflight decisions, pass those IDs with `--decisions <comma-separated-applicable-IDs>`; omission keeps the default fail-closed gate over every active applicable decision. Its exact commit is the final evidence boundary for the reviewed code. A final planning checkpoint does not accept a subset. For an entirely markerless graph, preserve the legacy closeout and do not infer a ledger, coverage aggregation, or Planning checkpoint.
...
8. After the bounded review and all applicable fix batches are validated, ensure the union of final-history trailers and validated ticket evidence covers every applicable preflight decision that declares `verification`. Then close a declared Planning context with `coverage aggregate` against the final reviewed head and those decision IDs, followed by `checkpoint --phase implementation`. The aggregator filters selected decisions without `verification`, keeps ticket-only mode valid, and fails closed when selected verification evidence is missing. Skip this closeout for an entirely markerless graph so its legacy path remains unchanged.
