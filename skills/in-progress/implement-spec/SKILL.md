...
8. After the bounded review and all applicable fix batches are validated, ensure the union of final-history trailers and validated ticket evidence covers every applicable decision ID returned by preflight. Then close a declared Planning context with `coverage aggregate` against the final reviewed head and those decision IDs, followed by `checkpoint --phase implementation`. Skip this closeout for an entirely markerless graph so its legacy path remains unchanged.
