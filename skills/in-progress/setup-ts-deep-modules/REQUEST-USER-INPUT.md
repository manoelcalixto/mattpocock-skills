...
This file is the canonical authoring source. The skill that creates a **Decision prompt** points to a synchronized, co-located `REQUEST-USER-INPUT.md` so standalone copies remain self-contained. Run `npm run sync:request-user-input` after changing this contract or adding an owner; validation rejects divergence. A skill that delegates the interaction to another skill inherits the owner's behavior and carries no copy.
