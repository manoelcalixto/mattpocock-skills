...
- **[implement-spec](./implement-spec/SKILL.md)**: Implement a whole spec from one validated Planning checkpoint, coordinate isolated ticket worktrees and evidence, then run one bounded review checkpoint on the integrated result before the PR moves to human review. User-invoked.
- **[pr](./pr/SKILL.md)**: Reference for a pull request body with a visual summary, before/after evidence, and an assessment of reversibility and blast radius. Model-invoked.
